...
calcular1 = function () {
  ...
  if (isNaN(nota1)) {
    mostrarTexto("lblError1", "DATOS INCORRECTOS, INGRESA UN NUMERO");
    existeError = true;
  } else {
  ...
  if (isNaN(nota2)) {
    mostrarTexto("lblError2", "DATOS INCORRECTOS, INGRESA UN NUMERO");
    existeError = true;
  } else {
  ...
  if (isNaN(nota3)) {
    mostrarTexto("lblError3", "DATOS INCORRECTOS, INGRESA UN NUMERO");
    existeError = true;
  } else {
...
esNotaValida = function (nota, idComponenteError) {
  hayErrores = false;
  if (isNaN(nota)) {
    mostrarTexto(idComponenteError, "DATOS INCORRECTOS, INGRESA UN NUMERO");
    hayErrores = true;
  }
  if (nota < 0 || nota > 10) {
    mostrarTexto(idComponenteError, "DATOS INCORRECTOS, INGRESA UN NUMERO ENTRE 0 Y 10");
    hayErrores = true;
  }
  if (hayErrores == false) {
